...
To probe for acquisition traces, we need a dataset that comes with relevant annotations. Fortunately, this can be extracted from images' Exif metadata. Unfortunately, many readily available datasets don't provide this metadata. Thus, we use the Flickr API<d-footnote><a href="https://www.flickr.com/services/api/">https://www.flickr.com/services/api/</a></d-footnote> to search for safe-for-work, permissively licensed photos and extract their Exif metadata to create our image-label pairs. We present this dataset, named FlickrExif, below <span style="color: red;">NOTE: FlickrExif is still private; ILIAS is shown below as a placeholder</span>:
...
/* Column 1: fixed icon with header */
.topk-column {
  display: flex;
  flex-direction: column;
  align-items: center;
...
.topk-pair-grid {
  display: grid;
  grid-template-columns: 1fr 1fr; /* match headers */
  row-gap: 0.5rem;                
}
...
.topk-pair-row.active .topk-pair-icon img {
  opacity: 1;                     
}

/* Middle fixed image */
.topk-fixed-image img {
  width: 80px;
  height: auto;
  display: block;
  margin: auto;
...
<div class="l-page">
<div class="topk-layout">

  <!-- Left columns -->
  <div class="topk-left-columns">
    
    <!-- Column 1: fixed icon -->
    <div class="topk-column">
      <div class="topk-column-title">query</div>
      <div class="topk-fixed-icon-wrapper">
        <div class="topk-fixed-icon">
          <img src="/assets/img/camera_bias/q_75.png">
        </div>
      </div>
    </div>

    <!-- Column 2: selectable pairs -->
    <div class="topk-selectable">
      <!-- Headers -->
      <div class="topk-pair-headers">
        <div>positives</div>
        <div>negatives</div>
      </div>

      <!-- Icon pairs -->
      <div class="topk-pair-grid">
        <div class="topk-pair-row active" data-folder="/assets/img/camera_bias/baseline">
          <div class="topk-pair-icon"><img src="/assets/img/camera_bias/p_75.png" alt="B"></div>
          <div class="topk-pair-icon"><img src="/assets/img/camera_bias/n_75.png" alt="C"></div>
        </div>
        <div class="topk-pair-row" data-folder="/assets/img/camera_bias/pos">
          <div class="topk-pair-icon"><img src="/assets/img/camera_bias/p_75.png" alt="C"></div>
          <div class="topk-pair-icon"><img src="/assets/img/camera_bias/n_95.png" alt="D"></div>
        </div>
        <div class="topk-pair-row" data-folder="/assets/img/camera_bias/neg">
          <div class="topk-pair-icon"><img src="/assets/img/camera_bias/p_95.png"></div>
          <div class="topk-pair-icon"><img src="/assets/img/camera_bias/n_75.png"></div>
        </div>
      </div>
    </div>
  </div> <!-- end topk-left-columns -->

  <!-- Middle fixed image -->
  <div class="topk-fixed-image">
    <img src="/assets/img/camera_bias/query.jpg">
  </div>

  <!-- Right strip of 10 images -->
  <div class="topk-strip" id="topk-strip"></div>
</div>
</div>

<script>
const imageStrip = document.getElementById("topk-strip");
const rows = document.querySelectorAll(".topk-pair-row");

// initial load
loadImages("/assets/img/camera_bias/baseline");
...
rows.forEach(row => {
  row.addEventListener("click", () => {
    ...
    loadImages(row.dataset.folder);
  });
});

function loadImages(folder) {
  const files = Array.from({length: 10}, (_, i) => `${folder}/${i}.jpg`);
  imageStrip.innerHTML = files.map(src => `<img src="${src}" alt="">`).join("");
}
</script>
...
To do this, we collect a dataset we dub PairCams, available <a href="https://huggingface.co/datasets/ryanramos/PairCams">here</a>. We capture 730 pairs of photos of the *exact same subject* with nearly *identical shooting conditions* (e.g. angle, camera orientation, time of day, camera shooting mode), with the difference between camera type. We experiment with modern smartphones and older digital cameras. Despite the difference in camera, the image pairs contain nearly identical semantic content, meaning that it should be trivial to retrieve one using the other.
...
<div class="figcaption">Recall@$$1$$ on the <i>same</i> and <i>different</i> collections. Zoom into the square for a clearer look. Again, CVL models show the most sensitivity, with SSL models showing the least. A plot on $$ y=x $$ implies completely robust performance.</div>
