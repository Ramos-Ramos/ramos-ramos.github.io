...
To probe for acquisition traces, we need a dataset that comes with relevant annotations. Fortunately, this can be extracted from images' Exif metadata. Unfortunately, many readily available datasets don't provide this metadata. Thus, we use the Flickr API<d-footnote><a href="https://www.flickr.com/services/api/">https://www.flickr.com/services/api/</a></d-footnote> to search for safe-for-work, permissively licensed photos and extract their Exif metadata to create our image-label pairs. We present this dataset, named FlickrExif, below:
